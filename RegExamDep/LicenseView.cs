...
sealed class LicenseView : View
{
    ...
    public LicenseView()
    {
        ExamDays = new Dictionary<DateOnly, bool>(365);
        DateOnly now = DateOnly.FromDateTime(DateTime.Today);
        for (int dayCounter = 0; dayCounter < 365; dayCounter++)
        {
            ExamDays[now.AddDays(dayCounter)] = false;
        }
    }

    public bool ChooseExamDay(DateOnly day)
    {
        if (!ExamDays.TryGetValue(day, out bool occupied))
        {
            return false;
        }
    ...
    public void PrintExamDays()
    {
        ...
        while (new DateOnly(currentYear, currentMonth, 1) < end)
        {
            ...
            for (int day = 1; day <= daysInMonth; day++)
            {
                ...
                else if (ExamDays.TryGetValue(currentDate, out bool occupied) && occupied)
                {
                    Console.Write("--  ");
                }
                else
